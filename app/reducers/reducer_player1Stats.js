...
export default function(state = INITIAL_STATE, action) {
  ...
  switch(action.type) {
    ...
    case MINUS_AUTHORITY_P1:
      if (
        lastItem &&
        lastItem.type === 'authority' &&
        lastItem.operator === '-'
      ) {
        lastItem.val += 1;
        return {
          ...state,
          authority: state.authority - 1,
    ...
    case P2_ATTACKS:
      if ( action.payload >= state.defense ) {
        ...
        return {
          ...state,
          authority: state.authority - action.payload,
          history: []
        };
      } else {
